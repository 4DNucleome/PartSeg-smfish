...
class GaussBackgroundEstimateParameters(BaseModel):
    ...
    estimate_mask: bool = Field(True, description="Estimate background outside mask")
...
class LayerRangeThresholdFlowParameters(CellFromNucleusFlow.__argument_class__):
    lower_layer: int = Field(0, title="Lower layer", ge=-1, le=1000, position=0)
    upper_layer: int = Field(-1, title="Lower layer", ge=-1, le=1000, position=1)
...
class ThresholdFlowAlgorithmParametersWithDilation(ThresholdFlowAlgorithmParameters):
    dilation_radius: int = Field(
        0,
        title="Dilation radius",
        description="Radius of dilation applied to the mask",
        ge=0,
        le=100,
    )


class ThresholdFlowAlgorithmWithDilation(ThresholdFlowAlgorithm):
    new_parameters: ThresholdFlowAlgorithmParametersWithDilation
    __argument_class__ = ThresholdFlowAlgorithmParametersWithDilation

    @classmethod
    def get_name(cls) -> str:
        return "Threshold Flow with Dilation"

    @classmethod
    def get_steps_num(cls) -> int:
    ...
    def calculation_run(
        ...
        report_fun("Dilate ROI", self.get_steps_num() - 1)
        ...
        res2 = ROIExtractionResult(
            roi=roi,
            parameters=self.get_segmentation_profile(),
            additional_layers=deepcopy(res.additional_layers),
        )
